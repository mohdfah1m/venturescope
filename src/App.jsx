...
// ─── HELPERS ─────────────────────────────────────────────────────────────────
const getScoreColor = (s) => s >= 85 ? "#00ff9d" : s >= 70 ? "#ffd166" : "#ff6b6b";
...
const Tag = ({ label }) => (
  <span style={{
    ...
    borderRadius: 4, padding: "2px 7px", fontSize: 11, letterSpacing: 0.3, whiteSpace: "nowrap"
  }}>{label}</span>
);
...
const StageChip = ({ stage }) => {
  ...
  return (
    <span style={{
      ...
      borderRadius: 4, padding: "1px 8px", fontSize: 11, fontWeight: 600, letterSpacing: 0.5, whiteSpace: "nowrap"
    }}>{stage}</span>
  );
};
  ...
  return (
    <aside style={{
      ...
      <div style={{ padding: "16px 20px", borderBottom: "1px solid #1e1e24" }}>
        ...
        <div style={{ fontSize: 11, color: "#555" }}>mock_companies.csv</div>
...
const BottomNav = ({ active, setActive, lists }) => {
  const items = [
    { id: "companies", label: "Companies", icon: "◉" },
    { id: "lists", label: "Lists", icon: "⊞", badge: lists.length },
  ...
  return (
    <nav style={{
      ...
      background: "#0d0d0f", borderTop: "1px solid #1e1e24",
      ...
      {items.map(item => (
        <button key={item.id} onClick={() => setActive(item.id)} style={{
          ...
          color: active === item.id ? "#a78bfa" : "#555",
          fontSize: 10, fontWeight: active === item.id ? 700 : 400,
          letterSpacing: 0.3, position: "relative", transition: "color 0.15s"
        }}>
          ...
          {active === item.id && (
            <span style={{
              ...
              height: 2, background: "#6366f1", borderRadius: "0 0 2px 2px"
            }} />
          )}
        </button>
      ))}
...
// ─── COMPANY CARD (mobile list item) ─────────────────────────────────────────
const CompanyCard = ({ company, onSelect }) => (
  <div onClick={() => onSelect(company)} style={{
    ...
    onTouchStart={e => e.currentTarget.style.borderColor = "#6366f140"}
    onTouchEnd={e => e.currentTarget.style.borderColor = "#1e1e24"}
  >
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
      <div>
        <div style={{ fontSize: 14, fontWeight: 700, color: "#e5e5e5" }}>{company.name}</div>
    ...
    <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
      ...
      <span style={{ fontSize: 11, color: "#555" }}>{company.sector}</span>
      <span style={{ fontSize: 11, color: "#444", marginLeft: "auto" }}>{company.lastActivity}</span>
    </div>
  </div>
);
...
const CompaniesPage = ({ onSelect, lists, addToList }) => {
  ...
  const handleBulkList = () => {
    if (!lists.length) return alert("Create a list first.");
  ...
  const Pagination = () => pages > 1 ? (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, marginTop: 16 }}>
      <button onClick={() => setPage(p => Math.max(0, p - 1))} disabled={page === 0} style={{
        width: 32, height: 32, borderRadius: 6, border: "1px solid #2a2a33", background: "transparent",
        color: page === 0 ? "#333" : "#888", cursor: page === 0 ? "not-allowed" : "pointer",
      ...
      <button onClick={() => setPage(p => Math.min(pages - 1, p + 1))} disabled={page === pages - 1} style={{
        width: 32, height: 32, borderRadius: 6, border: "1px solid #2a2a33", background: "transparent",
        color: page === pages - 1 ? "#333" : "#888", cursor: page === pages - 1 ? "not-allowed" : "pointer",
        fontSize: 16, display: "flex", alignItems: "center", justifyContent: "center"
      }}>›</button>
    </div>
  ) : null;
  ...
  return (
    <div style={{ padding: isMobile ? "16px" : "28px 24px", fontFamily: "'DM Sans', sans-serif", minWidth: 0, paddingBottom: isMobile ? 80 : "28px" }}>
      <div style={{ marginBottom: 16 }}>
        <h1 style={{ fontSize: isMobile ? 20 : 22, fontWeight: 700, color: "#f5f5f5", margin: 0, letterSpacing: -0.5 }}>Companies</h1>
        <div style={{ color: "#555", fontSize: 12, marginTop: 3 }}>{filtered.length} companies · CSV</div>
      </div>

      {/* Search */}
      <input value={search} onChange={e => { setSearch(e.target.value); setPage(0); }}
        placeholder="Search companies, sectors…"
        style={{ width: "100%", marginBottom: 10, background: "#16161a", border: "1px solid #2a2a33", borderRadius: 8, padding: "9px 12px", color: "#e5e5e5", fontSize: 13, outline: "none", fontFamily: "inherit", boxSizing: "border-box" }} />
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
        {[["stage", STAGES, stage, v => { setStage(v); setPage(0); }],
          ...
          <select key={key} value={val} onChange={e => setter(e.target.value)} style={{
            flex: 1, minWidth: 90, background: "#16161a", border: "1px solid #2a2a33", borderRadius: 8,
            padding: "8px 8px", color: val.startsWith("All") ? "#555" : "#e5e5e5",
      ...
      {selected.size > 0 && (
        <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", background: "#6366f115", border: "1px solid #6366f130", borderRadius: 8, marginBottom: 12 }}>
          ...
          <button onClick={() => setSelected(new Set())} style={{ background: "transparent", border: "1px solid #333", color: "#888", borderRadius: 6, padding: "5px 10px", fontSize: 12, cursor: "pointer", fontFamily: "inherit" }}>Clear</button>
        </div>
      )}
      ...
      {isMobile ? (
        <div>
          {visible.map(c => <CompanyCard key={c.id} company={c} onSelect={onSelect} />)}
          <Pagination />
        </div>
      ) : (
        <>
...
const CompanyProfile = ({ company, onBack, lists, addToList }) => {
  ...
  return (
    <div style={{ padding: isMobile ? "16px" : "28px 24px", fontFamily: "'DM Sans', sans-serif", maxWidth: "100%", minWidth: 0, paddingBottom: isMobile ? 80 : "28px" }}>
      <button onClick={onBack} style={{ background: "transparent", border: "none", color: "#555", cursor: "pointer", fontSize: 13, display: "flex", alignItems: "center", gap: 6, marginBottom: 20, fontFamily: "inherit", padding: 0 }}>
        ← Back
      </button>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 20, gap: 12 }}>
        <div style={{ display: "flex", gap: 12, alignItems: "flex-start", minWidth: 0 }}>
          <div style={{ width: 44, height: 44, borderRadius: 10, background: "linear-gradient(135deg, #6366f1, #a78bfa)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, fontWeight: 900, color: "#fff", flexShrink: 0 }}>
            {company.name[0]}
          </div>
          <div style={{ minWidth: 0 }}>
            <h2 style={{ margin: 0, fontSize: isMobile ? 18 : 22, fontWeight: 700, color: "#f5f5f5", letterSpacing: -0.5 }}>{company.name}</h2>
            <div style={{ color: "#555", fontSize: 12, marginTop: 2 }}>{company.hq} · {company.founded}</div>
            <div style={{ marginTop: 6, display: "flex", gap: 5, flexWrap: "wrap" }}>
              <StageChip stage={company.stage} />
              {company.tags.slice(0, isMobile ? 2 : 4).map(t => <Tag key={t} label={t} />)}
            </div>
          </div>
        </div>
        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 8, flexShrink: 0 }}>
          <div style={{ fontSize: 26, fontWeight: 800, color: getScoreColor(company.score), fontFamily: "monospace" }}>{company.score}</div>
          <div style={{ position: "relative" }}>
            <button onClick={() => setShowListMenu(!showListMenu)} style={{ background: "#6366f1", border: "none", color: "#fff", borderRadius: 7, padding: "7px 12px", fontSize: 12, cursor: "pointer", fontFamily: "inherit", fontWeight: 600, whiteSpace: "nowrap" }}>
              + List ▾
            </button>
            {showListMenu && (
              <div style={{ position: "absolute", top: "100%", right: 0, marginTop: 4, zIndex: 100, background: "#18181d", border: "1px solid #2a2a33", borderRadius: 8, padding: 8, minWidth: 150 }}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 20, flexWrap: "wrap" }}>
        {[["Funding", company.funding], ["Employees", company.employees], ["Sector", company.sector], ["Country", company.country]].map(([k, v]) => (
          <div key={k} style={{ background: "#111114", border: "1px solid #1e1e24", borderRadius: 8, padding: "10px 14px", flex: "1 1 100px", minWidth: 80 }}>
            <div style={{ fontSize: 10, color: "#444", letterSpacing: 0.5, marginBottom: 3 }}>{k.toUpperCase()}</div>
      ...
      {/* Tabs */}
      <div style={{ display: "flex", gap: 0, marginBottom: 20, borderBottom: "1px solid #1e1e24", overflowX: "auto" }}>
        {["overview", "signals", "enrich", "notes"].map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)} style={{
            ...
            letterSpacing: 0.3, marginBottom: -1, transition: "all 0.15s", whiteSpace: "nowrap"
      ...
      {activeTab === "overview" && (
        <div>
          <div style={{ fontSize: 14, color: "#bbb", lineHeight: 1.7, marginBottom: 16 }}>{company.description}</div>
          {inLists.length > 0 && (
            <div>
        ...
        <div>
          {company.signals.map((sig, i) => (
            <div key={i} style={{ background: "#111114", border: "1px solid #1e1e24", borderRadius: 8, padding: "12px 16px", display: "flex", alignItems: "center", gap: 12, marginBottom: 10 }}>
              ...
              <div style={{ marginLeft: "auto", fontSize: 11, color: "#444", whiteSpace: "nowrap" }}>{["2d ago", "5d ago", "1w ago"][i] || "recently"}</div>
            </div>
          ))}
        </div>
      )}
        ...
        <div>
          {!enriched && !enriching && (
            <div style={{ textAlign: "center", padding: "40px 0" }}>
              ...
              <div style={{ color: "#888", fontSize: 14, marginBottom: 20 }}>Enrich with live data from {company.domain}</div>
          ...
          {enriching && (
            <div style={{ textAlign: "center", padding: "40px 0" }}>
              <div style={{ fontSize: 13, color: "#6366f1" }}>Fetching {company.domain}…</div>
              <div style={{ fontSize: 11, color: "#444", marginTop: 8 }}>Scraping homepage, about, careers</div>
            </div>
          )}
          ...
          {enriched && (
            <div>
              ...
              {[
                ...
                ["WHAT THEY DO", enriched.whatTheyDo.map((b, i) => <div key={i} style={{ display: "flex", gap: 8, marginBottom: 6, fontSize: 13, color: "#bbb" }}><span style={{ color: "#6366f1", flexShrink: 0 }}>·</span>{b}</div>)],
                ["KEYWORDS", <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>{enriched.keywords.map(k => <Tag key={k} label={k} />)}</div>],
                ["DERIVED SIGNALS", enriched.signals.map((s, i) => <div key={i} style={{ display: "flex", gap: 8, marginBottom: 6, alignItems: "center" }}><span style={{ fontSize: 10, color: s.type === "positive" ? "#00ff9d" : "#ffd166", flexShrink: 0 }}>{s.type === "positive" ? "●" : "○"}</span><span style={{ fontSize: 13, color: "#bbb" }}>{s.label}</span></div>)],
                ["SOURCES", enriched.sources.map((s, i) => <div key={i} style={{ fontSize: 11, marginBottom: 4 }}><span style={{ color: "#6366f1", wordBreak: "break-all" }}>{s.url}</span><span style={{ color: "#444", marginLeft: 8 }}>{new Date(s.scraped).toLocaleTimeString()}</span></div>)],
              ].map(([label, content]) => (
                <div key={label} style={{ background: "#111114", border: "1px solid #1e1e24", borderRadius: 8, padding: "14px", marginBottom: 12 }}>
      ...
      {activeTab === "notes" && (
        <div>
          <textarea value={note} onChange={e => setNote(e.target.value)} placeholder="Add notes about this company…"
            style={{ width: "100%", minHeight: 140, background: "#111114", border: "1px solid #2a2a33", borderRadius: 8, padding: 14, color: "#ccc", fontSize: 13, lineHeight: 1.7, fontFamily: "inherit", resize: "vertical", outline: "none", boxSizing: "border-box" }} />
...
const ListsPage = ({ lists, setLists }) => {
  ...
  return (
    <div style={{ padding: isMobile ? "16px" : "28px 24px", fontFamily: "'DM Sans', sans-serif", minWidth: 0, paddingBottom: isMobile ? 80 : "28px" }}>
      <h1 style={{ fontSize: isMobile ? 20 : 22, fontWeight: 700, color: "#f5f5f5", margin: 0, letterSpacing: -0.5, marginBottom: 20 }}>Lists</h1>

      <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
        <input value={newName} onChange={e => setNewName(e.target.value)} onKeyDown={e => e.key === "Enter" && createList()}
          placeholder="New list name…"
          style={{ flex: 1, background: "#16161a", border: "1px solid #2a2a33", borderRadius: 8, padding: "9px 12px", color: "#e5e5e5", fontSize: 13, outline: "none", fontFamily: "inherit" }} />
      ...
      <div style={{ display: "flex", gap: 16, flexDirection: isMobile ? "column" : "row" }}>
        <div style={{ width: isMobile ? "100%" : 220, flexShrink: 0 }}>
          {lists.map(l => (
            <div key={l.name} onClick={() => setActive(active === l.name && isMobile ? null : l.name)}
              style={{ padding: "10px 14px", borderRadius: 8, cursor: "pointer", marginBottom: 8, background: active === l.name ? "#6366f120" : "#111114", border: `1px solid ${active === l.name ? "#6366f140" : "#1e1e24"}` }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                {isMobile && <span style={{ color: "#555", fontSize: 12 }}>{active === l.name ? "▲" : "▼"}</span>}
              </div>

              {/* On mobile: expand inline */}
              {isMobile && active === l.name && (
                <div style={{ marginTop: 12, paddingTop: 12, borderTop: "1px solid #1e1e24" }}>
                  <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
                    <button onClick={e => { e.stopPropagation(); exportCSV(l); }} style={{ background: "#16161a", border: "1px solid #2a2a33", color: "#888", borderRadius: 6, padding: "6px 10px", fontSize: 11, cursor: "pointer", fontFamily: "inherit" }}>Export CSV</button>
                    <button onClick={e => { e.stopPropagation(); setLists(prev => prev.filter(x => x.name !== l.name)); setActive(null); }} style={{ background: "#ff6b6b15", border: "1px solid #ff6b6b30", color: "#ff6b6b", borderRadius: 6, padding: "6px 10px", fontSize: 11, cursor: "pointer", fontFamily: "inherit" }}>Delete</button>
                  </div>
                  {!l.companies.length ? <div style={{ color: "#444", fontSize: 13 }}>No companies yet.</div>
                    : l.companies.map(id => {
                      const c = MOCK_COMPANIES.find(x => x.id === id);
                      if (!c) return null;
                      return (
                        <div key={id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #18181d" }}>
                          <div>
                          ...
                          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                            ...
                            <button onClick={e => { e.stopPropagation(); setLists(prev => prev.map(x => x.name === l.name ? { ...x, companies: x.companies.filter(cid => cid !== id) } : x)); }}
...
const SavedPage = ({ onGoToCompanies }) => {
  ...
  return (
    <div style={{ padding: isMobile ? "16px" : "28px 24px", fontFamily: "'DM Sans', sans-serif", minWidth: 0, paddingBottom: isMobile ? 80 : "28px" }}>
      <h1 style={{ fontSize: isMobile ? 20 : 22, fontWeight: 700, color: "#f5f5f5", margin: 0, letterSpacing: -0.5, marginBottom: 20 }}>Saved Searches</h1>

      <div style={{ background: "#111114", border: "1px solid #1e1e24", borderRadius: 10, padding: 16, marginBottom: 20 }}>
        <div style={{ fontSize: 11, color: "#555", marginBottom: 10, letterSpacing: 0.5 }}>SAVE A SEARCH</div>
        <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", gap: 8 }}>
          <input value={name} onChange={e => setName(e.target.value)} placeholder="Name…"
            style={{ flex: isMobile ? "unset" : "0 0 160px", background: "#16161a", border: "1px solid #2a2a33", borderRadius: 8, padding: "9px 12px", color: "#e5e5e5", fontSize: 13, outline: "none", fontFamily: "inherit" }} />
          <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Query / filters…"
            style={{ flex: 1, background: "#16161a", border: "1px solid #2a2a33", borderRadius: 8, padding: "9px 12px", color: "#e5e5e5", fontSize: 13, outline: "none", fontFamily: "inherit" }} />
      ...
      {saves.map((s, i) => (
        <div key={i} style={{ background: "#111114", border: "1px solid #1e1e24", borderRadius: 8, padding: "14px 16px", marginBottom: 10 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10 }}>
            <div style={{ minWidth: 0 }}>
              <div style={{ fontSize: 14, color: "#e5e5e5", fontWeight: 600 }}>{s.name}</div>
              <div style={{ fontSize: 12, color: "#555", marginTop: 3, wordBreak: "break-word" }}>{s.query}</div>
              <div style={{ fontSize: 11, color: "#333", marginTop: 4 }}>{new Date(s.saved).toLocaleDateString()}</div>
            </div>
            <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
              <button onClick={onGoToCompanies} style={{ background: "#6366f115", border: "1px solid #6366f130", color: "#a78bfa", borderRadius: 6, padding: "6px 10px", fontSize: 12, cursor: "pointer", fontFamily: "inherit" }}>Run ↗</button>
              <button onClick={() => { const s2 = saves.filter((_, j) => j !== i); setSaves(s2); ls.set("saved_searches", s2); }}
                style={{ background: "transparent", border: "1px solid #2a2a33", color: "#555", borderRadius: 6, padding: "6px 10px", fontSize: 12, cursor: "pointer", fontFamily: "inherit" }}>Del</button>
            </div>
          </div>
        </div>
      ))}
...
export default function App() {
  ...
  const navigate = (p) => { setPage(p); setSelectedCompany(null); };
  const activePage = page === "profile" ? "companies" : page;
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", width: "100%", maxWidth: "100vw", overflow: "hidden", background: "#0a0a0d", color: "#f5f5f5", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      {/* Desktop sidebar */}
      {!isMobile && <Sidebar active={activePage} setActive={navigate} lists={lists} />}

      <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", overflow: "hidden" }}>

        {/* Top bar */}
        <div style={{ background: "#0d0d0f", borderBottom: "1px solid #1e1e24", padding: isMobile ? "10px 16px" : "12px 24px", display: "flex", alignItems: "center", gap: 12, position: "sticky", top: 0, zIndex: 50, flexShrink: 0 }}>
          {isMobile && (
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginRight: 4 }}>
              <div style={{ width: 26, height: 26, borderRadius: 6, background: "linear-gradient(135deg, #6366f1, #a78bfa)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 900, color: "#fff" }}>V</div>
              <span style={{ fontSize: 13, fontWeight: 700, color: "#f5f5f5" }}>VentureScope</span>
            </div>
          )}
          <input placeholder={isMobile ? "Search…" : "Global search… (⌘K)"}
            style={{ flex: 1, maxWidth: isMobile ? "100%" : 400, background: "#16161a", border: "1px solid #2a2a33", borderRadius: 8, padding: "7px 12px", color: "#ccc", fontSize: 13, outline: "none", fontFamily: "inherit" }} />
          {!isMobile && <div style={{ fontSize: 11, color: "#333", letterSpacing: 0.5, whiteSpace: "nowrap" }}>{MOCK_COMPANIES.length} COMPANIES · CSV SEED</div>}
        </div>
        ...
        <main style={{ flex: 1, overflowY: "auto", overflowX: "hidden" }}>
          ...
          {page === "saved" && <SavedPage onGoToCompanies={() => navigate("companies")} />}
        </main>
      </div>

      {/* Mobile bottom nav */}
      {isMobile && <BottomNav active={activePage} setActive={navigate} lists={lists} />}
    </div>
  );
}
